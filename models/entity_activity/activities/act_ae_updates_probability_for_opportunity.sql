...
with cte_opportunity_history as (
    select
        entity_id,
        probability_to_close as current_value,
        lag(probability_to_close, 1) over(partition by entity_id order by observed_on asc) as previous_value,
        {{ dbt_utils.star(from=ref('ent_opportunities_history'), except=['expected_value', 'entity_id'], quote_identifiers=False) }}
    from
